...
Chunk 策略：固定大小，句子，段落，递归(分隔符表格)，滑动窗口，语义(相似度)，父子(级联，树形)，文档格式分割(Document-Specific Chunking)
...
BDD style需求描述。RAG包含现有的系统文档。用AIReview需求清晰完整度->补充缺失部分->再次Review直到没有不明确的时候。继续下一步借助RAG更准确生成代码。
...
\###　11　Agentic模式
反思（reflection）－PDCA循环？
提示链(Prompt Chain)-流水线。
规划－混合模式－总分总
并行处理－就是上一个的凑数版本
路由－上一个的凑数版本
就三种：1 迭代模式就像;2瀑布模式;　3 混合模式

### 12 QA Pairs

QAGenerationChain
取样(调研)－生成Q(问题规划)-标注(答案准备)-测试、审查(Review)
QAParis质量标准：语义相似验证、长度验证、关键词匹配验证以及唯一验证

### 13 数据集生成

大模型测试－数据改写(大模型蒸馏的一种方式？)
Query 改写
同义词改写，改写 Query 筛选
ROUGE-L和BLEU归一化验证
帕累托（Vilfredo Pareto）微小改进
Todo：coding

### 14 AI赋能测试左移

开卡(Groming,需求澄清)，验卡(验收测试)
AI当评审
大模型友好度:模块化程度,遗留系统依赖性,迭代频率与动态性

### 15

### 16

流量录制回放、全链路压测、精准测试、缺陷逃逸
测试是质量工程的一部分，不是全部。这在前 AI时代已经在很多企业里面实践了。
